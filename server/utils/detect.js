...
const TIMEOUT_IN_MILLISECONDS = 5 * 1000;
const options = { times: 50, timeout: 500 };
...
async function networkRequest(website, family) {
  ...
  if (status) {
    ...
    let loss = Number(
      (response.filter(e => e.status == false).length / requestTimes).toFixed(5)
    );

    let dnsLookup = Number(
      (
        reached.map(e => e.result.dnsLookup).reduce((a, b) => a + b) /
        reachedCount
      ).toFixed(3)
    );
    let tcpConnection = Number(
      (
        reached.map(e => e.result.tcpConnection).reduce((a, b) => a + b) /
        reachedCount
      ).toFixed(3)
    );
    let tlsHandshake = url.parse(website).protocol.startsWith("http:")
      ...
      : Number(
          (
            reached.map(e => e.result.tlsHandshake).reduce((a, b) => a + b) /
            reachedCount
          ).toFixed(3)
        );
    ...
    let firstByte = Number(
      (
        reached.map(e => e.result.firstByte).reduce((a, b) => a + b) /
        reachedCount
      ).toFixed(3)
    );
    let contentTransfer = Number(
      (
        reached.map(e => e.result.contentTransfer).reduce((a, b) => a + b) /
        reachedCount
      ).toFixed(3)
    );
    let total = Number(
      (
        reached.map(e => e.result.total).reduce((a, b) => a + b) / reachedCount
      ).toFixed(3)
    );
    let docSize = Number(
      (
        reached.map(e => e.result.docSize).reduce((a, b) => a + b) /
        reachedCount
      ).toFixed(0)
    );
...
async function pingTest(address, family = 4) {
  ...
  let loss = alive
    ? Number(
        (result.filter(e => e.reached == false).length / result.length).toFixed(
          5
        )
      )
    : 100;
  let rttList = alive
    ? [...result.filter(e => e.reached == true).map(e => e.roundtriptime)]
    : null;
