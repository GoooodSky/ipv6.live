  ...
  methods: {
    ...
    tableHeaderFixed() {
      let tableHeader = document.querySelector(
        "#university-list .el-table__header-wrapper"
      );
      let tableHeaderTop = this.$getElementTop(tableHeader);
      window.addEventListener("scroll", () => {
        let universityListLen = document.querySelector(
          "#university-list .el-table"
        ).clientHeight;
        let scrollTop = document.documentElement.scrollTop;
        if (
          scrollTop > tableHeaderTop &&
          scrollTop < tableHeaderTop + universityListLen - 240
        ) {
          tableHeader.classList.add("tableHeaderFixed");
          tableHeader.parentNode.style["padding-top"] =
            tableHeader.offsetHeight + "px";
        } else {
