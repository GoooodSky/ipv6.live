...
export default {
  computed: {
    provinceList() {
      let provinces = Object.entries(this.$store.state.provincesDetail).map(province => {
        let [key, value] = province
        return {
          name: key,
          universityCount: value.universityCount,
          ipv6SupportCount: value.ipv6SupportCount,
